...
exports.registerUser = async (req, res) => {
  try {
    ...
    if (dupMessage) return res.status(409).json({ success: false, message: dupMessage });
...
exports.loginUser = async (req, res) => {
  try {
    ...
    // Set cookie (works for same-origin / localhost)
    ...
    // ✅ Also return token in body so frontend can store in memory/header
...
exports.getAllAlumni = async (req, res) => {
  try {
    ...
    const profileMap = {};
    for (const profile of profiles) profileMap[profile.user.toString()] = profile;
...
/* ─────────────────────────────────────────
   ...
   ✅ Accepts token from EITHER:
      1. Cookie (works on localhost / same-origin)
      2. Authorization: Bearer header (works cross-origin on Render)
───────────────────────────────────────── */
